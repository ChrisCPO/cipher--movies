...
describe TemporaryMovie do
  describe ".title" do
    it "returns title" do
      movie = TemporaryMovie.new(movie_in_test_response)
  ...
  describe ".id" do
    it "returns apples id" do
      ...
      expect(movie.id).to eq 909728081
    end
  end
  ...
  describe ".local_db_movie_info_hash" do
    it "returns hash for Movie" do
      movie = TemporaryMovie.new(movie_in_test_response)
      hash = {id: movie.id, title: movie.title}

      expect(movie.local_db_movie_info_hash).to eq hash
    end
  end
